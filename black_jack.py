...
blackjack_X, blackjack_Y = 860, 560
...
bj_msg = ""
bj_result_text = ""
bj_delay_timer = 0.0  # kurzer Delay bevor Dealer zieht
...
def blackjackspiel_logik(bj_state, events, dt=0.0):
    ...
    # --- bj_waiting: Startbildschirm / Warten auf Spielbeginn ---
    if bj_state == "bj_waiting":
        for e in events:
    ...
    if bj_state == "bj_result":
        for e in events:
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_SPACE:   # Neue Runde starten
                    bj_reset_round()
                    return "bj_player"
                if e.key == pygame.K_r:       # Explizit resetten
                    bj_reset_round()
                    return "bj_player"
        return bj_state
...
def blackjackspiel_zeichnen(bj_state, screen):
    ...
    dy = panel.y + 70
    if bj_state in ("bj_player", "bj_waiting"):
    ...
    # Steuerungs-Hinweise
    my = py + 40
    hint = "H=Hit  S=Stand  SPACE/R=Neue Runde  Q=Exit"
    if bj_state == "bj_waiting":
        hint = "Drücke eine Taste zum Starten (R=Reset  Q=Exit)"
    screen.blit(font_medium.render(hint, True, (200, 230, 255)), (panel.x + 20, my))
